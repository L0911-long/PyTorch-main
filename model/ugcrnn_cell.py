...
from model.layer_params import *
...
class UGCGRUCell(nn.Module):
    def __init__(self, num_units, adj_mx, max_diffusion_step, num_nodes, nonlinearity='tanh',
        ...
        if filter_type == "laplacian":
            ...
            self._supports = [self._build_sparse_matrix(support) for support in supports]
            ...
            if K == 2:
                ...
                for k in range(2, K):
                    multi_order_laplacian[k] = 2 * torch.mm(laplacian, multi_order_laplacian[k-1]) - \
                                               multi_order_laplacian[k-2] #  T_k(L) = 2 * L * T_{k-1}(L) - T_{k-2}(L)
    ...
    def forward(self, inputs, hx):
        ...
        if self.is_trans:
            c_gcn = self.transformer(c_gcn)
        # print(c_gcn.size(),'2222222222222222222')
        new_state = u * hx + (1.0 - u) * c_gcn
        return new_state
    ...
    def _fc(self, inputs, state, output_size, bias_start=0.0):  # GRU
